...
from app.games.battleship.game import BattleshipGame, BattleshipPhase
from app.games.battleship.keyboards import battleship_finished_keyboard
from app.games.battleship.presentation import (
    battleship_private_board_text,
    battleship_public_text,
    battleship_results_text,
    sync_battleship_ui,
)
...
async def _running(callback: CallbackQuery, session: AsyncSession, game_id: int, phase_seq: int):
    ...
    if resolved is None:
        return None
    game, _ = resolved
    if game.status != GameSessionStatus.RUNNING.value or game.phase != BattleshipPhase.TURN.value:
        await callback.answer("❌ Эта кнопка больше не активна.", show_alert=True)
        return None
...
@router.callback_query(F.data == "gm:rules:battleship")
async def battleship_rules(callback: CallbackQuery) -> None:
    await callback.answer(
        "🚢 Морской бой: два игрока получают скрытые поля 5×5. "
        "Флот расставляется автоматически. Ходите по очереди, нажимая координаты. "
        "Побеждает тот, кто первым уничтожит все корабельные клетки соперника.",
        show_alert=True,
    )


@router.callback_query(F.data.regexp(r"^gm:bs:\d+$"))
async def battleship_start(callback: CallbackQuery, bot: Bot, session: AsyncSession) -> None:
    ...
    try:
        ...
        await callback.answer("Для Морского боя нужны ровно 2 игрока.", show_alert=True)
        return
    except Exception:
        ...
        return
    await close_lobby_message(bot, session, group=group, game=game, text="▶️ 🚢 Морской бой начался.")
    latest = await manager.get_game(session, game_id=game.id)
    if latest is not None:
        await sync_battleship_ui(bot, session, latest)
    await callback.answer("▶️ Морской бой начался")


@router.callback_query(F.data.regexp(r"^gm:bf:\d+:\d+:\d+$"))
async def battleship_fire(callback: CallbackQuery, bot: Bot, session: AsyncSession) -> None:
    _, _, game_raw, phase_raw, cell_raw = (callback.data or "").split(":")
    ...
    if player is None or player.status != "alive":
        ...
        return
    try:
        result, created = await engine.fire(
            session,
            game,
        ...
        text = "❌ По этой клетке уже стреляли." if "already fired" in str(error) else "❌ Этот ход больше недоступен."
    ...
    if latest is not None:
        await sync_battleship_ui(bot, session, latest)
    if not created:
        await callback.answer("✅ Этот выстрел уже принят.", show_alert=True)
    else:
        await callback.answer("💥 Попадание!" if result == "hit" else "🌊 Мимо.", show_alert=True)


@router.callback_query(F.data.regexp(r"^gm:bm:\d+:\d+$"))
async def battleship_my_board(callback: CallbackQuery, session: AsyncSession) -> None:
    ...
    if player is None or player.status != "alive":
        await callback.answer("❌ Вы не участвуете в этой игре.", show_alert=True)
        return
    text = await battleship_private_board_text(session, game, callback.from_user.id)
    await callback.answer(text[:200], show_alert=True)


@router.callback_query(F.data.regexp(r"^gm:bc:\d+:\d+$"))
async def battleship_cancel(callback: CallbackQuery, bot: Bot, session: AsyncSession) -> None:
    _, _, game_raw, phase_raw = (callback.data or "").split(":")
    game = await _running(callback, session, int(game_raw), int(phase_raw))
    if game is None:
        return
    ...
    if group is None:
        return
    if callback.from_user.id != game.creator_telegram_id and not await can_manage_group(bot, group, callback.from_user.id, session):
...
@router.callback_query(F.data.regexp(r"^gm:bres:\d+$"))
async def battleship_results(callback: CallbackQuery, session: AsyncSession) -> None:
    game_id = int((callback.data or "").rsplit(":", 1)[-1])
    resolved = await _game_group(callback, session, game_id)
    ...
    markup = InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text="◀️ К итогу", callback_data=f"gm:bfinal:{game.id}")
    ]])
    await callback.message.edit_text(await battleship_results_text(session, game), reply_markup=markup)
    await callback.answer()


@router.callback_query(F.data.regexp(r"^gm:bfinal:\d+$"))
async def battleship_final(callback: CallbackQuery, session: AsyncSession) -> None:
    game_id = int((callback.data or "").rsplit(":", 1)[-1])
    resolved = await _game_group(callback, session, game_id)
    if resolved is None or callback.message is None:
        return
    ...
    await callback.message.edit_text(
        await battleship_public_text(session, game),
        reply_markup=battleship_finished_keyboard(game.id),
    )
    await callback.answer()
