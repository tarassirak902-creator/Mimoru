...
def test_rank_mutation_middleware_locks_only_reachable_mutations() -> None:
    source = (ROOT / "app/middlewares_rank_safety.py").read_text(encoding="utf-8")
    ...
    middleware = source.split("class RankMutationLockMiddleware", 1)[1]
...
def test_rank_mutation_middleware_is_scoped_to_telegram_roles() -> None:
    ...
    assert "telegram_roles.router.callback_query.middleware(rank_mutation_lock_middleware)" in main
    assert "telegram_roles.router.message.middleware(rank_mutation_lock_middleware)" in main
    assert "group_action_aliases.router.callback_query.middleware(rank_mutation_lock_middleware)" not in main
