...
def test_manual_plan_service_locks_group_before_expiry_mutation() -> None:
    source = (ROOT / "app/services/manual_plans.py").read_text(encoding="utf-8")
    apply_helper = source.split("async def apply_manual_plan", 1)[1]
    lock = apply_helper.index(".with_for_update()")
    ...
    assert lock < expiry_read
    assert lock < plan_write < event_add < commit
...
def test_live_service_plan_apply_uses_serialized_hardened_winner() -> None:
    ...
    assert "group = await apply_manual_plan(" in source
    assert "service_management_fixes.service_plan_apply_serialized" in audit


def test_all_manual_plan_callbacks_share_locked_mutation_service() -> None:
    source = (ROOT / "app/handlers/service_management_fixes.py").read_text(encoding="utf-8")
    ...
    assert "await apply_manual_plan(" in apply_handler
    assert "await apply_manual_plan(" in action_handler
...
def test_legacy_trial_text_uses_serialized_hardened_winner() -> None:
    ...
    assert "group = await apply_manual_plan(" in handler
