...
def test_help_pages_fit_telegram_message_limit() -> None:
    pages = (
        group_help_full._home_text(),
        group_help_full._members_text(),
        group_help_full._member_alias_text(),
        group_help_full._moderation_text(),
        group_help_full._admin_lists_text(),
        group_help_full._protection_text(),
        group_help_full._management_text(),
        group_help_full._joins_text(),
        group_help_full._content_text(),
        group_help_full._admin_extra_text(),
        group_help_full._roles_text(),
    )
...
def test_games_slash_command_still_opens_existing_interactive_catalog() -> None:
    preferences = _source("app/handlers/fun_preferences.py")
    assert 'Command("games")' in preferences
    assert "fun_help.entertainment_help(message)" in preferences
